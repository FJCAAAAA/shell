...
# 说明：首先通过Cobbler Profile “RHEL7.3-x86_64-mini”，完成系统自动部署，然后执行本脚本完成bjape01-dmzngx1安装及配置。因为中间涉及IP地址修改，所以本脚本只能在控制台或后台执行。
# 因为要建立三节点的高可用集群，集群管理的命令在本节点执行即可，但需要另外的两个节点先完成节点定制并重启。
# 
# 注：本脚本适用于以下配置的服务器：
#     1、服务器为ESXi或KVM虚拟机，其他虚拟化平台虚拟机未经验证；
#     2、创建虚机时OS选择RHEL 7.3；
#     3、创建虚机时只需配置一块网卡，网卡首先配置“桥接br246”，等安装完成后再改成“br223”；
...
pcs resource create ${resourceName} ocf:heartbeat:nginx op monitor interval=10s
