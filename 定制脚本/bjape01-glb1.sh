...
# 说明：首先通过Cobbler Profile “RHEL7.3-x86_64-mini”，完成系统自动部署，然后执行本脚本完成bjape01-glb1安装及配置。因为中间涉及IP地址修改，所以本脚本只能在控制台或后台执行。
...
# 以下集群管理命令只需要在本节点执行即可，但需要另外的节点已经完成节点定制
pcs cluster auth ${env}-${node1} ${env}-${node2} ${env}-${node3} -u hacluster -p ${initPWD}
pcs cluster setup --name ${env}-${clusterName} ${env}-${node1} ${env}-${node2} ${env}-${node3}
pcs cluster enable --all
pcs cluster start --all
pcs resource create VirtIP ocf:heartbeat:IPaddr2 ip=${VIP} cidr_netmask=24 op monitor interval=10s
echo "------------------检查结果------------------"
pcs status
...
# 将haproxy服务添加为pcs集群资源。以下命令只需要在本节点执行即可，但需要另外的节点已经完成节点定制
pcs resource create ${resourceName} lsb:haproxy op monitor interval=10s
pcs constraint colocation add ${resourceName} with VirtIP INFINITY
pcs constraint order ${resourceName} then VirtIP
...
do
  pcs cluster stop ${env}-${node}
  pcs status
  pcs cluster start ${env}-${node}
  pcs status
done
...
do
  pcs resource move ${resourceName} ${env}-${node}
  pcs status
done
pcs resource move ${resourceName} ${env}-${localNode}
pcs status
...
# 删除临时网卡子接口
ifdown ${netIf}:2
rm -f /etc/sysconfig/network-scripts/ifcfg-${netIf}:2
echo "------------------检查结果------------------"
